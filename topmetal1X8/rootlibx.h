...
void run(char *pedefile,char *polNFitPar,char *polNFitParRev,char *TotalchargeFile,char *placData3,char *pointChargeOutFile,char *pointOutRoot,char *CanvasRoot){
	pointChargeOutFile=pointChargeOutFile;
	pointOutRoot=pointOutRoot;
 ...
 void beamSize(int ich, double upLimit, double lowLimit)
   {
	...
	double beamSizeCol;
	double beamSize;
	double beamSizeMean = 0;
	double beamSizeRMS = 0;
...
void beamPosFit_show_all_pixel_pd1(char *pedeFile, int ich, int iframe, double upLimit, double lowLimit)
   {//for 2d map max pixel fit
    pd1Pede pede;
    pede.setup(pedeFile); 
	...
	int iPoint=0;
		...
		ge.SetPoint(iPoint, (irow+1)*pitch, pixelMean );//gravity point turn 90 degree
		//Set ex, ey values for ipoint
		ge.SetPointError(iPoint, 0, sqrt(pixelSigma2));//gravity point error  bar
		geRota.SetPoint(iPoint, pixelMean, (irow+1)*pitch );//gravity point
		//geRota.SetPointError(iPoint, sqrt(pixelSigma2), 0);
		iPoint++;
	...
	TString hist2dNameRotat;
        ...
        TH2D *hAdc2dMapRotat= new TH2D(hist2dNameRotat, "", 72, 0*pitch, 72*pitch, 72, 0*pitch, 72*pitch);
            ...
            adc_value/=(double)pd1.samPerPix();
            adc_value-=pede.meanPed[ich*pd1.nPix()+ipixel];   
            arry2dmap2[binY][binX] = adc_value;
        }
      	...
      	for (int k = 0; k < 72; k++)
    {
      for (int l = 0; l < 72; l++)
      {
		hAdc2dMapRotat->SetBinContent(l+1,k+1,arry2dmap2[k][l]); 
      }
   }
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	char *beamPosFitNamePng =Form("pd1png2/beamPositionFitChip%dFrame%d_show_all_pixel.png",ich, iframe);
        cBeamPosFit->SaveAs(beamPosFitNamePng);
...
///////////////////for pd1 test, end of function:beamPosFit_show_all_pixel_pd1////////////////////////////////////////////////////////
...
void beamProfile_pd1(char *pedeFile, int ich, int iframe, double upLimit, double lowLimit)
   {//align straight and sum 72 row to fit
   	pd1Pede pede;
    pede.setup(pedeFile);
...

/////////////////////////new for pd1 test, begin of function:beamPosGausrowFit_pd1///////////////////////////////////////////
void beamPosGausrowFit_pd1(char *pedeFile, int ich, int iframe, double upLimit, double lowLimit)
   {//for single row fit
   	upLimit=upLimit;
   	lowLimit=lowLimit;
   	pd1Pede pede;
	...
	int binMove=0;
        ...
        TString hist2dNameRotat;
        hist2dNameRotat = "hAdc2dMap_iframe"; //hist2dNameRotat += iframe;
        TH2D *hAdc2dMapRotat= new TH2D(hist2dNameRotat, ";pixel row index;pixel column index", 72, 0*pitch, 72*pitch, 72, 0*pitch, 72*pitch);
	...
	// f->FixParameter(1, 1000);
	f->SetParLimits(1, (location_Num[irow] - Del_width) * pitch, (location_Num[irow] + Del_width) * pitch); 
	f->SetParLimits(2, 100, 250);//0 is normalization term, 1 is mean, 2 is sigma,
	beamProfileX->Fit(f,"B");// fit within specified range, use ParLimits
...
///////////////////for pd1 test, end of function:beamPosGausrowFit_pd1////////////////////////////////////////////////////////
//////////////////////////////////////////////lizili test  
...
void beamPosgausFit_pd1(char *pedeFile, int ich, int iframe, double upLimit, double lowLimit)
   {//for 2d map gaus fit
	...
	int iPoint=0;
		...
		// f->FixParameter(1, 1000);
		f->SetParLimits(1, (location_Num[irow] - Del_width) * pitch, (location_Num[irow] + Del_width) * pitch); 
		f->SetParLimits(2, 100, 250);//0 is normalization term, 1 is mean, 2 is sigma,[0]*exp(-0.5*((x-[1])/[2])**2)
		beamProfileX->Fit(f,"B");// fit within specified range, use ParLimits
		GausMean[irow]=f->GetParameter(1);
		cout<< "row_" <<irow << "=" << GausMean[irow] <<endl;

		//Set x, y values for ipoint
		ge.SetPoint(iPoint, (irow+1)*pitch, GausMean[irow] );//gravity point turn 90 degree
		//Set ex, ey values for ipoint
		// ge.SetPointError(iPoint, 0, sqrt(pixelSigma2));
		geRota.SetPoint(iPoint, GausMean[irow], (irow+1)*pitch );//center of gravity point
		//geRota.SetPointError(iPoint, sqrt(pixelSigma2), 0);
		iPoint++;
	...
	TF1 *fit1 = new TF1("fit1", "pol1", 2*pitch, 71*pitch);
    ge.Fit("fit1", "QR");//fit ge and geRota
	b = fit1->GetParameter(0);//value of 0th parameter,intercept of fit line
	k = fit1->GetParameter(1);//value of 1th parameter,slope of fit line
	theta = atan(k)*180/TMath::Pi();
	...
	cout<<"theta: "<<(90-theta)<<endl;
	char *beamPosFitNamePng =Form("pd1png/beamPosgausFitChip%dFrame%d.png",ich, iframe);
        cBeamPosFit->SaveAs(beamPosFitNamePng);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////new for pd1 test, begin of function:beamPosFit_pd1///////////////////////////////////////////
void beamAngleResol_pd1(char *pedeFile, int ich, int iframe, double upLimit, double lowLimit)
   {//for angle fit max pixel method
    pd1Pede pede;
    pede.setup(pedeFile);
    ...
    int location_Num[72];  
    int Del_width =5;  
    double xrow[72]={0},ycolumn[72]={0};
    double xxrow[72][2]={0},yycolumn[72][2]={0};
	...
	int iPoint=0;
		...
		geRota.SetPoint(iPoint, pixelMean, (irow+1)*pitch );//gravity point
		//geRota.SetPointError(iPoint, sqrt(pixelSigma2), 0);
		xrow[iPoint]=pixelMean;
		ycolumn[iPoint]=(irow+1)*pitch;
		iPoint++;
    ...
    TCanvas *cBeamPosFit = new TCanvas("cBeamPosFit", "Beam Position Fit", 1600 ,800);	
    TH1D *h1Angle = new TH1D("h1Angle", "Angle", 100, 80, 90);
    cBeamPosFit->Divide(2,1);
	cBeamPosFit->cd(1);
    cBeamPosFit->SetCanvasSize(1600, 800);
	//hAdc2dMapRotat90->Draw("COLZ");
    hAdc2dMapRotat->Draw("COLZ");//draw 72*72 2d map
	...
	for(int i=0;i<35;i++){
	xxrow[i][0]=xrow[i];
	yycolumn[i][0]=ycolumn[i];
	xxrow[i][1]=xrow[i+35];
	yycolumn[i][1]=ycolumn[i+35];
	const int n = 2;
	// TGraphErrors *connect2point = new TGraphErrors(n,xxrow[i],yycolumn[i]);
	TGraph *connect2point = new TGraph(n,xxrow[i],yycolumn[i]);
	connect2point->SetLineColor(i);
	connect2point->Draw("same c");//draw line between two points
	if(xxrow[i][1]!=xxrow[i][0]){
	theta[i] = atan2((xxrow[i][1]-xxrow[i][0]),(yycolumn[i][1]-yycolumn[i][0]))*180/TMath::Pi();
	}
	else{
	...
	cout<<"thetaRMS: "<<thetaRMS<<endl;
	cout<<"thetaResol: "<<thetaRMS/sqrt(nPoints)<<endl;

	h1Angle->GetXaxis()->SetTitle("Angle (#circ)");
	h1Angle->GetXaxis()->CenterTitle();
	h1Angle->GetYaxis()->SetTitle("Counts");
	h1Angle->GetYaxis()->CenterTitle();
	cBeamPosFit->cd(2);	
	h1Angle->Draw();//draw histgram
...
///////////////////for pd1 test, end of function:beamAngleResol_pd1////////////////////////////////////////////////////////
//////////////////////////////////////////////lizili test  
